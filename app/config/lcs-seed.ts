...
export function createLcsFreightSeed(): Record<string, FreightRecord[]> {
  ...
  const users = stampAll(base.users, LCS_ORG_ID, BRANCH_BAVET_ID).map((row, index) => ({
    ...
    organization: 'HollyWing Motor', branch: index === 2 ? 'All branches' : 'Bavet', telegram: index === 0 ? '@hollywing.admin' : '',
    roleAssignments: [{ role: row.role, organization: 'HollyWing Motor', branch: index === 2 ? 'All branches' : 'Bavet', effectiveDate: '2026-01-01', expiryDate: '', assignedBy: 'System Administrator' }],
  ...
  return {
    ...
    branches: [
      stamp({ id: 'br-001', branchCode: 'PPH', name: 'Phnom Penh', place: 'Phnom Penh', address: 'St. 271, Phnom Penh', phone: '+855 23 555 101', email: 'shop@hollywing.local', headOffice: 'Yes', status: 'Active' } as FreightRecord, LCS_ORG_ID, BRANCH_BAVET_ID),
      stamp({ id: 'br-002', branchCode: 'SR', name: 'Siem Reap', place: 'Siem Reap', address: 'Siem Reap, Cambodia', phone: '+855 63 555 102', email: 'sr@hollywing.local', headOffice: 'No', status: 'Active' } as FreightRecord, LCS_ORG_ID, BRANCH_PP_ID),
    ...
    systemSettings: [
      stamp({ id: 'set-001', organizationName: 'HollyWing Motor', branchName: '', settingKey: 'default_currency', settingValue: 'USD', displayValue: 'USD', scope: 'Organization', sensitive: 'No', updatedBy: 'System Administrator', updatedAt: '2026-08-20T08:00:00' } as FreightRecord, LCS_ORG_ID, BRANCH_BAVET_ID),
      stamp({ id: 'set-002', organizationName: 'HollyWing Motor', branchName: 'Phnom Penh', settingKey: 'telegram_bot_token', settingValue: '••••••••', displayValue: '••••••••', scope: 'Organization', sensitive: 'Yes', updatedBy: 'System Administrator', updatedAt: '2026-08-18T10:00:00' } as FreightRecord, LCS_ORG_ID, BRANCH_BAVET_ID),
    ],
  }
}
