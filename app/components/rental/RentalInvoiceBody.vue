...
const companyAddress = computed(() => tenant.activeOrganization?.address || DEFAULT_ADDRESS)
const companyPhone = computed(() => tenant.activeOrganization?.phone || DEFAULT_PHONE)
const companyEmail = computed(() => tenant.activeOrganization?.email || DEFAULT_EMAIL)
const companyContact = computed(() => [companyPhone.value, companyEmail.value].filter(Boolean).join(' · '))
</script>
