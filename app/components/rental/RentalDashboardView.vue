...
const fleetCards = computed<KpiCard[]>(() => [
  ...
  { key: 'progressing', title: tx('rental.dashboard.progressing', 'Progressing'), value: motorcycles.value.filter(row => ['Progressing', 'Rented'].includes(String(row.status))).length, to: '/motorcycles?status=Progressing' },
