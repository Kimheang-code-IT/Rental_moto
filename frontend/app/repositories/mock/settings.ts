...
import { DEFAULT_TELEGRAM_TEMPLATE, NOTIFICATION_EVENTS } from '~/types/rental/settings'

const INFO_KEY = 'rental:settings:app-info'
const CONFIG_KEY = 'rental:settings:app-config'
const STORAGE_KEY = 'rental:settings:storage'
...
function seedAppInfo(): AppInfo {
  return {
    ...
    shortName: 'HWM',
    businessName: 'HollyWing Motor',
    description: 'Motorcycle rental management system',
    supportEmail: 'support@hollywingmotor.com',
    supportPhone: '+855 23 000 000',
    website: 'https://rental.local',
    ...
    footer: {
      copyrightText: `© ${new Date().getFullYear()} HollyWing Motor`,
...
function seedAppConfig(): AppConfig {
  return {
    ...
    email: {
      ...
      fromEmail: 'noreply@rental.local',
      replyToEmail: '',
      timeoutSeconds: 30,
    ...
    telegram: {
      ...
      inlineKeyboardEnabled: true,
      showTransactionsButton: true,
      showMotorcycleStatusButton: true,
      showFinanceSummaryButton: true,
...
function seedStorage(): StorageProvider[] {
  ...
  return [
    ...
    },
    {
      ...
      accessMode: 'private',
      uploadPathPattern: 'rental/{yyyy}/{mm}/{filename}',
      endpoint: '',
      region: 'auto',
      bucket: '',
      accessKey: '',
    ...
    },
    {
      ...
      allowedFileTypes: ['pdf', 'docx', 'xlsx', 'png', 'jpg'],
      accessMode: 'private',
      uploadPathPattern: 'HollyWing Motor/{yyyy}/{mm}',
      folderId: '',
  ...
  return {
    ...
    async testTelegramConnection() {
      ...
      if (result.status === 'connected' && !value.telegram.botUsername) {
        value.telegram.botUsername = '@rental_mock_bot'
      }
      writeJson(CONFIG_KEY, value)
      return result
    },
