...
<template>
  <article v-if="rental" class="rental-invoice-sheet bg-white text-[#172033]">
    <header class="grid grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] items-start gap-4 border-b border-[#172033] pb-4">
      <div class="flex min-w-0 items-center gap-3">
        <img
          src="/logo.png"
          alt="Company logo"
          class="h-20 w-20 shrink-0 rounded-full object-contain"
          onerror="this.style.display='none'"
        >
        <div class="min-w-0">
          <p class="truncate text-[20px] font-extrabold text-[#0b4f91]">{{ companyName }}</p>
      ...
      <div class="min-w-0 text-right text-[12px] leading-snug text-slate-700">
        <p>{{ companyAddress }}</p>
    ...
    <section class="py-4">
      <div class="mb-4 text-center">
        <p class="text-[18px] font-extrabold leading-tight text-[#101827]">{{ L.invoice.km }}</p>
        <p class="text-[15px] font-black uppercase tracking-[0.14em] text-[#101827]">{{ L.invoice.en }}</p>
      </div>

      <div class="grid grid-cols-2 gap-8">
        <div>
          <h2 class="mb-2 text-[9px] font-extrabold uppercase tracking-wide text-slate-500">
          ...
          <dl class="space-y-2 text-[12px]">
            <div class="grid grid-cols-[8rem_1fr] gap-2">
              <dt class="leading-tight">
                ...
                <span class="block text-[10px] text-slate-500">{{ L.invoiceNo.en }}</span>
              </dt>
            ...
            <div class="grid grid-cols-[8rem_1fr] gap-2">
              <dt class="leading-tight">
                ...
                <span class="block text-[10px] text-slate-500">{{ L.paymentMethod.en }}</span>
              </dt>
            ...
            <div class="grid grid-cols-[8rem_1fr] gap-2">
              <dt class="leading-tight">
                ...
                <span class="block text-[10px] text-slate-500">{{ L.created.en }}</span>
              </dt>
          ...
          <dl class="space-y-2 text-[12px]">
            <div class="grid grid-cols-[7rem_1fr] gap-2">
              <dt class="leading-tight">
                ...
                <span class="block text-[10px] text-slate-500">{{ L.customer.en }}</span>
              </dt>
            ...
            <div class="grid grid-cols-[7rem_1fr] gap-2">
              <dt class="leading-tight">
                ...
                <span class="block text-[10px] text-slate-500">{{ L.returnDate.en }}</span>
              </dt>
    ...
    <table class="invoice-lines w-full table-fixed border-collapse text-[11px]">
      ...
      <thead>
        <tr class="bg-[#2463df] text-white">
          <th class="px-2 py-2 text-center font-bold leading-tight">
            ...
            <span class="block text-[9px] font-semibold opacity-90">{{ L.no.en }}</span>
          </th>
          <th class="px-2 py-2 text-center font-bold leading-tight">
            <span class="block">{{ L.motorcycle.km }}</span>
            <span class="block text-[9px] font-semibold opacity-90">{{ L.motorcycle.en }}</span>
          </th>
          <th class="px-2 py-2 text-center font-bold leading-tight">
            <span class="block">{{ L.plate.km }}</span>
            <span class="block text-[9px] font-semibold opacity-90">{{ L.plate.en }}</span>
          </th>
          <th class="px-2 py-2 text-center font-bold leading-tight">
            <span class="block">{{ L.days.km }}</span>
            <span class="block text-[9px] font-semibold opacity-90">{{ L.days.en }}</span>
          </th>
          <th class="px-2 py-2 text-center font-bold leading-tight">
      ...
      <tbody>
        <tr v-for="(item, index) in lineItems" :key="`${item.motorcycle}-${item.plate}-${index}`">
          <td class="border border-slate-200 px-2 py-2 text-center tabular-nums">{{ index + 1 }}</td>
    ...
    <section class="mt-4 grid grid-cols-[1fr_18rem] gap-6">
      <div class="text-[11px] leading-relaxed text-slate-600">
        ...
        <p class="mt-2">{{ L.paymentTerms.km }}</p>
    ...
    <footer class="mt-6 bg-[#2463df] px-4 py-3 text-center text-[11px] font-bold text-white">
